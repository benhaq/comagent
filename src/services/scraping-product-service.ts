...
const RETRY_SCHEDULE = Schedule.exponential("2 seconds").pipe(Schedule.compose(Schedule.recurs(2)))
...
function makeScrapingImpl(cache: {
  ...
  const fetchSearch = (query: string): Effect.Effect<ScrapingSearchResponse, ScrapingServiceUnavailable> =>
    Effect.tryPromise({
      try: async () => {
        const url = `${baseUrl}/api/search?q=${encodeURIComponent(query)}`
        const res = await fetch(url, { signal: AbortSignal.timeout(FETCH_TIMEOUT_MS) })
        if (!res.ok) throw new Error(`Scraping API ${res.status}: ${res.statusText}`)
  ...
  const search = (params: ProductSearchParams): Effect.Effect<ProductSearchResult, ScrapingServiceUnavailable> =>
    Effect.gen(function* () {
      ...
      // Client-side filters (scraping API only supports query)
      ...
      for (const sp of data.products) {
        yield* cache.set(productCacheKey(sp.asin), JSON.stringify(sp), CACHE_TTL_SECONDS).pipe(Effect.catchAll(() => Effect.void))
      }
      // Cache search results
      yield* cache.set(searchCacheKey(params.query), JSON.stringify(data), CACHE_TTL_SECONDS).pipe(Effect.catchAll(() => Effect.void))

      logger.debug({ query: params.query, total: data.total, returned: products.length, execMs: data.executionTime }, "Scraping search complete")
      return { products, totalResults: data.total, query: params.query }
    })

  const getDetails = (
    productId: string
  ): Effect.Effect<ProductDetail, ProductNotFound | ScrapingServiceUnavailable> =>
    Effect.gen(function* () {
      // Try cache first
