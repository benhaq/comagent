...
export interface ChatSessionServiceShape {
  create(userId: string, title?: string): Effect.Effect<ChatSession, DatabaseError>
  list(
    userId: string,
    limit: number,
    offset: number,
  ...
  delete(
    sessionId: string,
    userId: string,
  ): Effect.Effect<void, SessionNotFound | SessionOwnershipError | DatabaseError>
  addMessage(
    sessionId: string,
    role: string,
    content: unknown,
  ): Effect.Effect<ChatMessage, DatabaseError>
  autoTitle(sessionId: string): Effect.Effect<string, DatabaseError | AIServiceError>
}
